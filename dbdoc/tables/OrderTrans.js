...
var relations = [
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'order_trans_ibfk_3' } }}
	]
}),	
		new joint.dia.Link({
	source: { id: classes.order_trans.id },
...
_.each(relations, function(r) { r.toBack(); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonOrderTransGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonOrderTransGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
