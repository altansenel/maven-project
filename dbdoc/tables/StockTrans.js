...
stock_trans: new uml.BDDTable({
        ...
        methods: [
								...
								stock_trans_currency: new uml.BDDTable({
		id:'stock_trans_currency',
        position: { x:725.7759995228321  , y: 581.4035394871543 },
        size: { width: 220, height: 104 },
        name: 'stock_trans_currency',
        attributes: [
										'id: INT',
																															],
        methods: [
...
var relations = [
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'stock_trans_ibfk_6' } }}
	]
}),	
		new joint.dia.Link({
	source: { id: classes.stock_trans.id },
	...
	target: { id: classes.stock_trans.id },
	vertices: [],
	smooth: false,
	attrs: {
	'.marker-target': { d: 'M 20 0 L 0 5 L 20 10 z' }
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'stock_trans_detail_ibfk_1' } }}
	]
}),	
																																																																																																								new joint.dia.Link({
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'stock_trans_factor_ibfk_1' } }}
	]
}),	
																																																																																																																																																																																																new joint.dia.Link({
	source: { id: classes.stock_trans_tax.id },
	target: { id: classes.stock_trans.id },
	vertices: [],
	smooth: false,
	attrs: {
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'stock_trans_tax_ibfk_1' } }}
	]
}),	
																																																																																																																																																																																												new joint.dia.Link({
	source: { id: classes.stock_trans_currency.id },
	target: { id: classes.stock_trans.id },
	vertices: [],
	smooth: false,
	attrs: {
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'stock_trans_currency_ibfk_1' } }}
	]
}),	
																																																																																															];
...
_.each(relations, function(r) { r.toBack(); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonStockTransGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonStockTransGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
