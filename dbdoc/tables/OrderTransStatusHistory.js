...
var uml = joint.shapes.uml;
   ...
   var classes = {  
order_trans_status_history: new uml.BDDTable({
		id:'order_trans_status_history',
        position: { x:450  , y: 350 },
        size: { width: 180, height: 116 },
        name: 'order_trans_status_history',
        attributes: [
										'id: INT',
																																													],
        methods: [
...
_.each(classes, function(c) { graph.addCell(c); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonOrderTransStatusHistoryGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonOrderTransStatusHistoryGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
