...
var relations = [
	new joint.dia.Link({
	source: { id: classes.invoice_trans_status.id },
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'invoice_trans_status_ibfk_1' } }}
	]
}),	
...
_.each(relations, function(r) { r.toBack(); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonInvoiceTransStatusGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonInvoiceTransStatusGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
