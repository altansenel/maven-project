...
var uml = joint.shapes.uml;
   ...
   var classes = {  
admin_extra_fields: new uml.BDDTable({
		id:'admin_extra_fields',
        position: { x:450  , y: 350 },
        size: { width: 180, height: 116 },
        name: 'admin_extra_fields',
        attributes: [
										'id: INT',
																																													],
        methods: [
																	'idno: INT',
														'distinction: VARCHAR(15)',
														'name: VARCHAR(12)',
														'is_required: BIT',
														'is_active: BIT',
											]
...
_.each(classes, function(c) { graph.addCell(c); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonAdminExtraFieldsGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonAdminExtraFieldsGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
