...
var relations = [
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'invoice_trans_detail_ibfk_5' } }}
	]
}),	
		new joint.dia.Link({
	source: { id: classes.invoice_trans_detail.id },
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'invoice_trans_detail_ibfk_6' } }}
	]
}),	
		new joint.dia.Link({
	source: { id: classes.invoice_trans_detail.id },
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'invoice_trans_detail_ibfk_9' } }}
	]
}),	
...
_.each(relations, function(r) { r.toBack(); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonInvoiceTransDetailGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonInvoiceTransDetailGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
