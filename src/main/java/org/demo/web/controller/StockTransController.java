...
public class StockTransController extends AbstractController {

	//--- Variables names ( to be used in JSP with Expression Language )
	private static final String MAIN_ENTITY_NAME = "stockTrans";
	private static final String MAIN_LIST_NAME   = "list";

	//--- JSP pages names ( View name in the MVC model )
	private static final String JSP_FORM   = "stockTrans/form";
	private static final String JSP_LIST   = "stockTrans/list";

	//--- SAVE ACTION ( in the HTML form )
	private static final String SAVE_ACTION_CREATE   = "/stockTrans/create";
	private static final String SAVE_ACTION_UPDATE   = "/stockTrans/update";
	...
	public StockTransController() {
		super(StockTransController.class, MAIN_ENTITY_NAME );
		log("StockTransController created.");
	}
	 ...
	 * @param stockTrans
	 */
	private void populateModel(Model model, StockTrans stockTrans, FormMode formMode) {
		//--- Main entity
		model.addAttribute(MAIN_ENTITY_NAME, stockTrans);
	...
	//--------------------------------------------------------------------------------------
	/**
	 * Shows a list with all the occurrences of StockTrans found in the database
	...
	/**
	 * Shows a form page in order to create a new StockTrans
	 * @param model Spring MVC model
	 * @return
	 */
	@RequestMapping("/form")
	public String formForCreate(Model model) {
		...
		populateModel( model, stockTrans, FormMode.CREATE);
		return JSP_FORM;
	}

	/**
	 * Shows a form page in order to update an existing StockTrans
	 * @param model Spring MVC model
	...
	public String formForUpdate(Model model, @PathVariable("id") Integer id ) {
		...
		StockTrans stockTrans = stockTransService.findById(id);
		populateModel( model, stockTrans, FormMode.UPDATE);		
		return JSP_FORM;
	}
	 ...
	 * @param stockTrans  entity to be created
	...
	public String create(@Valid StockTrans stockTrans, BindingResult bindingResult, Model model, RedirectAttributes redirectAttributes, HttpServletRequest httpServletRequest) {
		log("Action 'create'");
		try {
			if (!bindingResult.hasErrors()) {
				StockTrans stockTransCreated = stockTransService.create(stockTrans); 
				model.addAttribute(MAIN_ENTITY_NAME, stockTransCreated);

				//---
				messageHelper.addMessage(redirectAttributes, new Message(MessageType.SUCCESS,"save.ok"));
				return redirectToForm(httpServletRequest, stockTrans.getId() );
			} else {
				populateModel( model, stockTrans, FormMode.CREATE);
				return JSP_FORM;
			}
			...
			messageHelper.addException(model, "stockTrans.error.create", e);
			populateModel( model, stockTrans, FormMode.CREATE);
			return JSP_FORM;
		}
	}
	 ...
	 * This action is based on the 'Post/Redirect/Get (PRG)' pattern, so it ends by 'http redirect'<br>
	 * @param stockTrans  entity to be updated
	 * @param bindingResult Spring MVC binding result
	 * @param model Spring MVC model
	 * @param redirectAttributes Spring MVC redirect attributes
	 * @param httpServletRequest
	...
	public String update(@Valid StockTrans stockTrans, BindingResult bindingResult, Model model, RedirectAttributes redirectAttributes, HttpServletRequest httpServletRequest) {
		log("Action 'update'");
		try {
			if (!bindingResult.hasErrors()) {
				//--- Perform database operations
				StockTrans stockTransSaved = stockTransService.update(stockTrans);
				model.addAttribute(MAIN_ENTITY_NAME, stockTransSaved);
			...
			messageHelper.addException(model, "stockTrans.error.update", e);
	...
	public String delete(RedirectAttributes redirectAttributes, @PathVariable("id") Integer id) {
		...
		try {
			...
			messageHelper.addException(redirectAttributes, "stockTrans.error.delete", e);
		}
